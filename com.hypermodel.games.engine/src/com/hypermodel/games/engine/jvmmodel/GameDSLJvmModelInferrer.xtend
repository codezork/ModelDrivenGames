				...
				acceptor.accept(
					...
					[
						...
						acceptor.accept(
							resizeClass,
							[inner|
								var innermethod = it.toMethod("onResize", _typeReferenceBuilder.typeRef(Void::TYPE), [
									parameters += it.toParameter("event", _typeReferenceBuilder.typeRef(ResizeEvent))
									annotations += _annotationTypesBuilder.annotationRef(Override)
									body = [
										append(
										'''
			...
			case ProjectType.ios: {
				...
				acceptor.accept(
					...
					[
						...
						it.members += method
						method = game.toMethod("main", _typeReferenceBuilder.typeRef(IOSApplication), [
							parameters += game.toParameter("arg", _typeReferenceBuilder.typeRef(typeof(String[])))
							body = [
								append(
								'''
						        NSAutoreleasePool pool = new NSAutoreleasePool();
						        UIApplication.main(argv, null, «game.name.toFirstUpper»Launcher.class);
						        pool.close();''')
							]
						])
						it.members += method
					...
					[
						...
						method = game.toMethod("main", _typeReferenceBuilder.typeRef(IOSApplication), [
							...
							body = [
								append(
								'''
								UIKit.UIApplicationMain(0, null, null, «game.name.toFirstUpper»Launcher.class.getName());''')
							]
						])
						it.members += method
					]
